...
df.columns
df.info()
columns = ["NUM_" + col.upper() if df[col].nunique() > 20 and df[col].dtypes in ["float64","int64"] else col.upper() for col in df.columns]
print(columns)
...
columns = [col.upper() + "FLAG" if "no" not in col else col.upper() for col in df.columns]
print(columns)
...
new_df = pd.DataFrame({col:df[col] for col in new_cols})
print(new_df)
#df1 = pd.DataFrame({col : df[col] for col in df.columns if col not in og_list})
#print(df1)
# # Notlar:
